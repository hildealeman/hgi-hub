...
interface TopicDiscussionCardProps {
  slug: string;
  cardTitle: string;
  cardEyebrow?: string;
  cardSummary: string;
...
function getOutlineClass(origin: Origin) {
  switch (origin) {
    case "original":
      return "ring-1 ring-sky-500/70";
    case "ai_accepted":
      return "ring-1 ring-emerald-500/70";
    case "edited_after_ai":
      return "ring-1 ring-amber-400/70";
    case "debated_with_ai":
      return "ring-2 ring-offset-2 ring-offset-zinc-950 ring-fuchsia-400";
  }
}

export function TopicDiscussionCard({
  slug,
  cardTitle,
  cardEyebrow,
  cardSummary,
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      ...
      try {
        ...
        if (topicError || !topic) {
          setError("No pudimos cargar el topic de esta conversación.");
          return;
        }
  ...
  const handlePublish = async (mode: Origin) => {
    ...
    try {
      ...
      const finalContent =
        mode === "ai_accepted" && aiSuggestion ? aiSuggestion : draft;
  ...
  return (
    <>
      <button
        ...
        className="text-left"
      >
        <Card title={cardTitle} subtle>
          {cardEyebrow && (
            <p className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">
              {cardEyebrow}
            </p>
          )}
          <p className="mt-1 text-sm text-zinc-200">{cardSummary}</p>
        </Card>
      </button>
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-4 overflow-hidden p-4 sm:flex-row sm:p-6">
                <div className="sm:w-1/2 space-y-3 overflow-y-auto pr-1">
                  <p className="text-sm text-zinc-200">{modalIntro}</p>
                  <div>{children}</div>
                </div>
                ...
                <div className="sm:w-1/2 flex flex-1 flex-col gap-3 overflow-y-auto">
                  ...
                  <div className="mt-2 space-y-2 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3 text-xs text-zinc-200">
                    ...
                    <textarea
                      ...
                      placeholder="¿Qué agregarías o cuestionarías de esta sección?"
                    />
                    ...
                    <div className="flex flex-wrap gap-2 pt-1 text-[11px]">
                      ...
                      <button
                        ...
                        className="rounded-full bg-sky-500 px-3 py-1 text-xs font-medium text-black transition-colors hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                      ...
                      <button
                        ...
                        className="rounded-full bg-emerald-500 px-3 py-1 text-xs font-medium text-black transition-colors hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                      ...
                      <button
                        ...
                        disabled={loading || !draft.trim()}
                        className="rounded-full bg-amber-400 px-3 py-1 text-xs font-medium text-black transition-colors hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                    ...
                    <p className="pt-1 text-[10px] text-zinc-500">
                      Más adelante activamos el modo arcoíris para debatir con la IA
                      antes de publicar (debated_with_ai).
                    </p>
                  </div>
                </div>
              </div>
